...
np.save(open('p2_train_labels.npy', 'w'), train_labels)

test_CSV = pd.read_csv(DIR + 'test.csv')
test_labels = test_CSV['label'].values[0:test_steps * batch_size]
np.save(open('p2_test_labels.npy', 'w'), test_labels)
...
np.save(open('p2_bottleneck_features_train.npy', 'w'), train_predictions)

test_predictions = intermediate_layer_model.predict_generator(generator_test, test_steps, verbose=1)
np.save(open('p2_bottleneck_features_test.npy', 'w'), test_predictions)

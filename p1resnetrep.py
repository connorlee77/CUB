...
train_labels = train_CSV['label'].values[0:test_steps * batch_size]
np.save(open('train_labels.npy', 'w'), test_predictions)

test_CSV = pd.read_csv(DIR + 'test.csv')
test_labels = test_CSV['label'].values[0:test_steps * batch_size]
np.save(open('test_labels.npy', 'w'), test_predictions)
...
generator_train = testgen.flow_from_directory(
    validation_data_dir,
    target_size=(img_width, img_height),
    batch_size=batch_size,
    class_mode='categorical',
...
model.load_weights('resnet_total.h5')
...
np.save(open('p1_bottleneck_features_train.npy', 'w'), train_predictions)

test_predictions = intermediate_layer_model.predict_generator(generator_test, test_steps, verbose=1)
np.save(open('p1_bottleneck_features_test.npy', 'w'), test_predictions)
